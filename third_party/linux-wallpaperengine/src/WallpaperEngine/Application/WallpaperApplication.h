...
public:
    ...
    /**
     * AnisPaper: expose the render context for offscreen frame capture
     */
    [[nodiscard]] WallpaperEngine::Render::RenderContext& getRenderContext () const;
    /**
     * AnisPaper: callback invoked after every rendered frame (offscreen capture hook)
     */
    void setFrameCallback (std::function<void (WallpaperApplication&)> callback);
    /**
     * Renders a frame
     */
...
private:
    ...
    /** AnisPaper: offscreen frame capture hook */
    std::function<void (WallpaperApplication&)> m_frameCallback = nullptr;
};
} // namespace WallpaperEngine::Application
