...
namespace {
// Older Workshop scene exporters write text padding as either one scalar or a
// two-axis string (for example "32.00000 32.00000").  The current model has
// one integer padding value and CText does not yet consume per-axis padding,
// so retain the horizontal value.  Invalid values are rejected locally and
// fall back to zero instead of aborting the whole Scene parse.
int parseTextPadding (const JSON& object) {
    const auto it = object.find ("padding");
    ...
    if (!it->is_string ()) {
	sLog.error ("Ignoring non-numeric text padding: ", it->dump ());
	return 0;
    }
    ::std::istringstream values (it->get<::std::string> ());
    double horizontal = 0.0;
    double vertical = 0.0;
    if (!(values >> horizontal) || !::std::isfinite (horizontal) ||
	horizontal < ::std::numeric_limits<int>::min () ||
	horizontal > ::std::numeric_limits<int>::max ()) {
	sLog.error ("Ignoring malformed text padding: ", it->dump ());
	return 0;
    }
    // A second numeric axis is valid but intentionally not represented by the
    // legacy single-value model.  Any non-whitespace trailing token is not.
    if (values >> vertical && (!::std::isfinite (vertical) ||
		vertical < ::std::numeric_limits<int>::min () ||
		vertical > ::std::numeric_limits<int>::max ())) {
	sLog.error ("Ignoring malformed text padding: ", it->dump ());
	return 0;
    }
    ::std::string trailing;
    ...
    return static_cast<int> (::std::lround (horizontal));
}
} // namespace
