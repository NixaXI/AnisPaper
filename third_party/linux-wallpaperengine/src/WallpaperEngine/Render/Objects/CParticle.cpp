...
namespace {
bool hasRenderableMaterial (const Particle& particle) {
    return particle.material && particle.material->material && !particle.material->material->passes.empty ()
        && particle.material->material->passes.front ();
}

// CRenderable stores a material reference and probes its first pass during construction.  A particle whose
// material failed to load must therefore receive a harmless placeholder until setupPass() can skip it.
const Material& particleRenderableMaterial (const Particle& particle) {
    if (hasRenderableMaterial (particle)) {
	return *particle.material->material;
    }

    static const Material fallback = [] {
	Material material {};
	material.filename = "<invalid-particle-material>";
	material.passes.emplace_back (std::make_unique<MaterialPass> ());
	return material;
    } ();
    return fallback;
}
...
CParticle::CParticle (Wallpapers::CScene& scene, const Particle& particle) :
    CObject (scene, particle), CRenderable (scene, particle, particleRenderableMaterial (particle)),
    ScriptableObject (scene, particle), m_particle (particle) {
    if (!hasRenderableMaterial (particle)) {
	sLog.error ("Skipping particle with no usable material: ", particle.name, " (", particle.particleFile, ")");
    }
...
void CParticle::setup () {
    ...
    if (hasRenderableMaterial (m_particle)) {
...
void CParticle::render () {
    if (!m_initialized || m_pass == nullptr || !m_particle.visible->value->getBool ()) {
	return;
    }
