...
// AnisPaper: CWeb.h is excluded (no CEF in the vendored build).
...
std::unique_ptr<CWallpaper> CWallpaper::fromWallpaper (
    ...
    if (wallpaper.is<Web> ()) {
	// AnisPaper: CEF is not vendored; web wallpapers are rendered by
	// AnisPaper's own Qt WebEngine child instead of this engine.
	sLog.exception ("Web wallpapers are not supported by the AnisPaper-curated scene engine");
    }
