...
void GLFWOpenGLDriver::dispatchEventQueue () {
    ...
    // For AnisPaper offscreen mode, pacing happens after the capture callback
    // in WallpaperApplication::render(). Keep the original driver pacing elsewhere.
    if (!this->m_context.settings.render.offscreen &&
	(endTime - startTime) < minimumTime) {
